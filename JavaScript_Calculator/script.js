document.addEventListener("DOMContentLoaded", () => {
  ...
  function processInputPress(buttonElement) {
    ...
    if (buttonElement.classList.contains("decimal")) {
      ...
      const currentWorkingSegment =
        activeStringParts[activeStringParts.length - 1];
    ...
    if (
      currentInputString === "0" &&
      !buttonElement.classList.contains("operator") &&
      !buttonElement.classList.contains("decimal")
    ) {
  ...
  function executeTerminalMathCalculation() {
    ...
    try {
      ...
      const computedMathOutcome = new Function(
        `return ${evaluationTargetString}`,
      )();

      // Format float output layout precision bounds to limit infinite loops strings
      const formattedTerminalValue = Number(
        computedMathOutcome.toFixed(8),
      ).toString();
